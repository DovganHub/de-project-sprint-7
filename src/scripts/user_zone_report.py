...
def user_zone_report() -> None:
    try:
        ...
        data_geo = spark.read\
            .parquet('hdfs://rc1a-dataproc-m-dg5lgqqm7jju58f9.mdb.yandexcloud.net:8020/user/aldovgan/data/geo/city_coordinates')
        ...
        act_city = spark.sql("select * from ordered_user_cities")\
            .where("rank = 1")\
        ...
        travel_data = spark.sql("select * from shift_data")\
            .select('user_id', 'act_city')\
            .groupBy('user_id')\
        ...
        logging.exception("An exception error!")
    finally:
        spark.stop()
